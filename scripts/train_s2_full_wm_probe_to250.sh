...
exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS2_full_wm_probe_to250 \
  --configs quadrotor --script bc_distill \
  --run.from_checkpoint outputs/dreamerv3/stageS2_head_only_probe_125/ckpt/20260826T171550F072027 \
  --run.from_checkpoint_regex '^(?!(bcopt|opt)/).*' \
  --run.bc_dataset outputs/dreamerv3/adaptation_episodes/stageS2_pos0p3_wm_probe_seed41000 \
  --run.bc_updates 125 --run.bc_validate_every 25 --run.bc_fixed_train False \
  ...
  --agent.imag_loss.actent 0.00001 --agent.imag_length 15 \
  --agent.report False \
  --jax.platform cpu --jax.prealloc False --jax.precompile True \
  --logger.outputs jsonl --logger.filter 'loss/(dyn|rep|vector|rew|con)|opt/' \
