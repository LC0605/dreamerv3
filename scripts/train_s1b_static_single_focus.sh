...
exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS1b_temporal3_static_single_focus \
  --configs quadrotor \
  ...
  --env.quadrotor.obstacle_count 1 --env.quadrotor.obstacle_spawn_probability 0.75 \
  ...
  --env.quadrotor.safety_distance 0.65 --env.quadrotor.safety_weight 4.0 \
  --env.quadrotor.ttc_reward_weight 4.0 \
  --env.quadrotor.safety_projection False \
  --agent.policy.minstd 0.03 --agent.imag_loss.actent 0.00001 --agent.opt.lr 0.000005 \
  --run.steps 20000 --run.envs 1 --run.train_ratio 64 \
