...
episodes=${3:-20}
for spec in random:91000:0.3:0.8:1 fixed0p8:70000:0.8:0.8:0 fixed0p6:71000:0.6:0.6:0 fixed0p3:72000:0.3:0.3:0 low:92000:0.3:0.45:1; do
  IFS=: read -r scene seed low high randomization <<< "$spec"
  logdir="outputs/dreamerv3/eval_${label}_${scene}"
  [[ ! -e "$logdir" ]] || { echo "Refusing to overwrite $logdir" >&2; exit 1; }
  DREAMER_EVAL_EPISODES="$episodes" /home/user/miniconda3/envs/dreamer_uav/bin/python \
    ...
    --env.quadrotor.random_offset_low "$low" --env.quadrotor.random_offset_high "$high" \
