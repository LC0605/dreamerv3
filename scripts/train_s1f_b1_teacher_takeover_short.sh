...
latest=$(tr -d '\n' < outputs/dreamerv3/stageS1fA3_temporal3_actor_mean_bc/ckpt/latest)
source_checkpoint="outputs/dreamerv3/stageS1fA3_temporal3_actor_mean_bc/ckpt/$latest"

exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS1fB1_temporal3_teacher_takeover_short \
  --configs quadrotor --script bc_distill \
  --run.from_checkpoint "$source_checkpoint" \
  --run.from_checkpoint_regex '^(?!bcopt/).*' \
  --run.bc_dataset outputs/dreamerv3/teacher_episodes/stage4_ppo_static_offset0p8_temporal3_seed20000 \
  --run.bc_updates 5 --run.bc_validate_every 5 --run.bc_fixed_train False \
  --batch_size 1 --batch_length 64 --report_length 64 --replay_context 0 \
  --agent.bc_only False --agent.bc_scale 1 --agent.bc_fixed_std 0.3 \
