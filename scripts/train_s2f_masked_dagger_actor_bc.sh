...
exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS2f_temporal3_masked_dagger_actor_bc \
  --configs quadrotor --script bc_distill \
  --run.from_checkpoint "$source_checkpoint" --run.from_checkpoint_regex '^(?!bcopt/).*' \
  --run.bc_dataset outputs/dreamerv3/teacher_episodes/stage5_level1_dagger_noise0p15_masked_failtail20 \
  --run.bc_updates 400 --run.bc_validate_every 100 --run.bc_fixed_train False \
  ...
  --agent.opt.lr 0.00004 --agent.opt.warmup 0 \
