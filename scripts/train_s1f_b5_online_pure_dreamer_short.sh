...
latest=$(tr -d '\n' < outputs/dreamerv3/stageS1fB4_temporal3_teacher_takeover_nobc/ckpt/latest)
source_checkpoint="outputs/dreamerv3/stageS1fB4_temporal3_teacher_takeover_nobc/ckpt/$latest"

exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS1fB5_temporal3_online_pure_short \
  --configs quadrotor --script train \
  --run.from_checkpoint "$source_checkpoint" --run.from_checkpoint_regex '^(?!bcopt/).*' \
  --run.steps 1000 --run.envs 1 --run.train_ratio 8 \
