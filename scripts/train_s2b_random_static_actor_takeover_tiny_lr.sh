...
latest=$(tr -d '\n' < outputs/dreamerv3/stageS2a3_temporal3_random_static_critic_only/ckpt/latest)
source_checkpoint="outputs/dreamerv3/stageS2a3_temporal3_random_static_critic_only/ckpt/$latest"

exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS2b_temporal3_random_static_actor_tiny_lr \
  ...
  --agent.freeze_policy False --agent.freeze_world_model True \
  --agent.loss_scales.policy 1 --agent.bc_only False --agent.bc_scale 0 \
  --agent.report False --agent.opt.lr 0.0000001 --agent.opt.warmup 0 \
  ...
  --env.quadrotor.seed 35400 --env.quadrotor.max_steps 300 \
