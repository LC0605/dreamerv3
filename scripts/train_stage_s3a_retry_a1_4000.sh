...
exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS3A_Retry_A1_4000 \
  --configs quadrotor --script bc_distill \
  --run.from_checkpoint outputs/dreamerv3/stageS2_Conservative_6000/ckpt/20260828T154503F849521 \
  --run.from_checkpoint_regex '.*' \
  --run.bc_dataset outputs/dreamerv3/stageS3A_Retry_A1_joint_replay \
  --run.bc_updates 4000 --run.bc_validate_every 1000 \
  --run.bc_fixed_train False --run.bc_risk_stratified False \
  --run.bc_scene_balanced False --run.bc_curriculum_new_fraction 0.60 \
  ...
  --env.quadrotor.obstacle_lateral_offset 0.0 \
