...
exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS2d_temporal3_random_success_actor_bc \
  --configs quadrotor --script bc_distill \
  --run.from_checkpoint "$source_checkpoint" --run.from_checkpoint_regex '^(?!bcopt/).*' \
  --run.bc_dataset outputs/dreamerv3/teacher_episodes/stage5_random_single_level1_success_only \
  --run.bc_updates 500 --run.bc_validate_every 100 --run.bc_fixed_train False \
