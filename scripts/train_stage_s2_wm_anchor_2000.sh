...
exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS2_WM_Anchor_2000 \
  ...
  --run.bc_dataset outputs/dreamerv3/stageS2_WM_anchor_offline_mixed \
  ...
  --agent.anchor_scale 10.0 \
  --agent.bc_only False --agent.bc_scale 0 --agent.bc_fixed_std 0 \
  --agent.freeze_encoder True --agent.freeze_dynamics False \
  ...
  --logger.filter 'loss/(dyn|rep|vector|rew|outcome|con|anchor)|anchor/|outcome/|opt/' \
