...
checkpoint=outputs/dreamerv3/stageS2_WM_Anchor_2000/ckpt/20260828T082718F144494
for spec in 0.8:70000 0.6:71000 0.3:72000; do
  offset=${spec%%:*}; seed=${spec##*:}; tag=${offset/./p}
  logdir=outputs/dreamerv3/eval_stageS2_WM_Anchor_offset_${tag}
  DREAMER_EVAL_EPISODES=100 /home/user/miniconda3/envs/dreamer_uav/bin/python \
    ...
    --run.from_checkpoint_regex '^(enc|dyn|dec|rew|outcome|con|pol|val|slowval)/' \
    --run.steps 100000 --run.envs 1 --run.debug True --run.log_every 300 \
    --agent.reward_decomposition True --agent.outcome_mode conditional_terminal \
    --agent.anchor_scale 0 --jax.platform cpu --jax.prealloc False \
