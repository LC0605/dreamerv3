...
exec /home/user/miniconda3/envs/dreamer_uav/bin/python dreamerv3/dreamerv3/main.py \
  --logdir outputs/dreamerv3/stageS2_Conservative_6000 \
  --configs quadrotor --script bc_distill \
  --run.from_checkpoint outputs/dreamerv3/stageS1fB2_temporal3_teacher_takeover_midbc/ckpt/20260825T222659F216085 \
  --run.from_checkpoint_regex '^(enc|dyn|dec|rew|con|pol|val|slowval|retnorm|valnorm|advnorm)/' \
  --run.bc_dataset outputs/dreamerv3/stageS2_Conservative_mixed_anchor_replay \
  --run.bc_updates 6000 --run.bc_validate_every 2000 \
  --run.bc_fixed_train False --run.bc_risk_stratified False \
  --run.bc_scene_balanced True \
  --batch_size 2 --batch_length 301 --report_length 301 --replay_context 0 \
  --agent.reward_decomposition False --agent.anchor_scale 1.0 \
  --agent.anchor_schedule_enabled True \
  --agent.anchor_schedule '2000.0,5.0,4000.0,2.0,5500.0,0.5,6000.0,0.0' \
  --agent.separate_actor_opt True \
