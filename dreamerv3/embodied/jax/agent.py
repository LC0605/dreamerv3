...
class Agent(embodied.Agent):
  ...
  def __init__(self, model, obs_space, act_space, config, jaxcfg):
    ...
    self._diagnose = transform.apply(
        nj.pure(self.model.diagnose), self.train_mesh,
        (tp, tm, ts), (ts,), ar, single_output=True, **shared_kwargs)
    self._anchor_actions = transform.apply(
        nj.pure(self.model.anchor_actions), self.train_mesh,
        (tp, tm, ts), (ts,), ar, single_output=True, **shared_kwargs)
  ...
  def train(self, carry, data):
    ...
    assert set(data) in (expected, expected | {'loss_mask'}), (
  ...
  def diagnose(self, data):
    """Run the model's read-only diagnostic graph without parameter updates."""
    with self.train_lock, jax.transfer_guard('allow'):
      data = internal.device_put(data, self.train_sharded)
      seed = self._seeds(int(self.n_batches), self.train_mirrored)
      outs = self._diagnose(self.params, seed, data)
  ...
  def anchor_actions(self, data):
    with self.train_lock, jax.transfer_guard('allow'):
      data = internal.device_put(data, self.train_sharded)
      seed = self._seeds(int(self.n_batches), self.train_mirrored)
      outs = self._anchor_actions(self.params, seed, data)
      return self._take_outs(internal.fetch_async(outs))

  def stream(self, st):
    def fn(data):
      # Online replay contains only real transitions. Offline fixed-shape
      # episode loaders may provide an explicit mask for padded rows.
      if 'loss_mask' not in data:
        data['loss_mask'] = np.ones_like(data['reward'], dtype=bool)
      if 'risk_stratum' not in data:
        data['risk_stratum'] = np.zeros_like(data['reward'], dtype=np.int32)
      if self.model.config.reward_decomposition:
        if 'reward_dense' not in data or 'terminal_outcome' not in data:
          raise KeyError('reward decomposition requires reward_dense and terminal_outcome')
