...
def main(argv=None):
  ...
  elif config.script == 'bc_distill':
    embodied.run.bc_distill(
...
def make_env(config, index, **overrides):
  ...
  ctor = {
      ...
      'quadrotor': 'embodied.envs.quadrotor:Quadrotor',
  }[suite]
  ...
  if suite == 'quadrotor' and os.environ.get('DREAMER_DISTINCT_ENV_SEEDS'):
    kwargs['seed'] = int(kwargs.get('seed', 0)) + int(index)
